...
int main()
{
    ...
    /* 0.3 - allocate the matrix & the results vectors */    
    ...
    /* EL vector ha de tenir mida Q=N^2/B^2 com a màxim en el cas que apliquem l'algorisme per blocs i N en el cas
    que apliquem el seqüèncial sense blocs. UTilitzem dos vectors diferents, un per a cada cas. */
    Q = (N^2)/(B^2); 
    printf("Q = %d",Q);
    printf("**********DEBUG***********");
    vector = malloc(N * sizeof(int)); 
    //vectorBlocks = malloc(Q * sizeof(int));
    ...
    resetVector(vectorBlocks,Q);
    sequentialBlockAlgorithmJORDI(vectorBlocks, matrix, N, B, X);
	printf("\n");
    printVector(vectorBlocks,Q);*/
    ...
    resetVector(vectorBlocks,Q); //WE DO REINITIALIZE THE VECTOR BEFORE TO USE IT AGAIN TO STORE THE RESULT OF OTHER FUNCTIONS 
    sequentialBlockAlgorithmJOSEP(vectorBlocks, matrix, N, B, X);
	printf("\n");
    printVector(vectorBlocks,Q);*/
    ...
    /* 2.1 - deallocate the matrix & vectors*/
    for (i=0; i<N; i++)
    {
        free(matrix[i]);
    }
    ...
    system("pause");
    //}
    // If there are no errors, the program should never reach this point (infinite loop sequential tasks scheduling)
    return 0;
}
...
void getInputParameters(int *N, int *B, int *X)
{
    ...
    while(1){
	    ...
	    // si no B i N no són multiples seguir amb el loop
	    	...
	    	system("cls");
			printf("B no es multiple de N. Try again");
			for (i=0;i<10;i++){
				printf("."); //LOADER....
				Sleep(250);
			}
...
void sequentialBlockAlgorithmJORDI(int* V, int** M, int N, int B, int x)
{
    ...
    b = 0; 
     ...
     /* Inicialitzem a zero el vector on desarem el sumatori dels elements de cada bloc.*/
    resetVector(V,N);
    ...
    for(i=0; i<N; i=i+B)
	{
		...
		for(j=0; j<N; j=j+B)
		{
			...
			/*Si no; en el cas quetots els elements contenen valors, 
